...
class CustomImageDataset(Dataset):
    def __init__(self, data_dir, img_size=(512,512)):
        ...
        for child_dir in os.listdir(data_dir):
            if os.path.isdir(os.path.join(data_dir, child_dir)):
                raw_img_dir = os.path.join(data_dir, child_dir, 'textRender.png')
                mask_img_dir = os.path.join(data_dir, child_dir, 'textRender_mask.png')
                label_dir = os.path.join(data_dir, child_dir, 'text.json')
                caption_dir = os.path.join(data_dir, child_dir, 'caption.txt')
                title_dir = os.path.join(data_dir, child_dir, 'title.txt')
                self.images.append(raw_img_dir)
                self.labels.append(label_dir)
                self.masks.append(mask_img_dir)
                self.captions.append(caption_dir)
                self.titles.append(title_dir)
    ...
    def __getitem__(self, idx):
        ...
        with open(self.captions[idx], "r") as f:
            caption = ','.join([line.strip() for line in f.readlines()])
        
        with open(self.titles[idx], "r") as f:
            raw_caption = ','.join([line.strip() for line in f.readlines()])

        # 获取文本标注
        jsf = json.load(open(self.labels[idx]))
...
if __name__ == '__main__':
    ...
    """
    args = {
        'data_dir': '/home/sxm/flux-workspace/FluxPoster/fakeTextData/outputs',
        'img_size': (512,512)
    }
