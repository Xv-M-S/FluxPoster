...
class XFluxPipeline:
    def __init__(self, model_type, device0, device1, offload: bool = False):
        self.device0 = torch.device(device0)
        self.device1 = torch.device(device1)
        self.device = self.device0
        ...
        if "fp8" in model_type:
            self.model = load_flow_model_quintized(model_type, device="cpu" if offload else self.device0)
        else:
            self.model = load_flow_model(model_type, device="cpu" if offload else self.device0)
    ...
    def forward(
        ...
        with torch.no_grad():
            if self.offload:
                self.t5, self.clip = self.t5.to(self.device1), self.clip.to(self.device1)
