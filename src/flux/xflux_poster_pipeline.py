...
class XFluxPipeline:
    def __init__(self, model_type, device, offload: bool = False):
        ...
        # 添加其他模块
        self.font_embedding = FontEmbedding(device = device).to(device)
        self.auxiliary = AuxiliaryLatentMoudle().to(device)
        self.attention_pooling = AttentionPooling(hidden_dim=768).to(device)
